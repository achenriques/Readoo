module.exports = {
    ...
    chatHistory: "SELECT c.chatId, c.userIdFrom, c.userIdTo, c.chatDateTime, " + 
            ...
            "INNER JOIN app_user u2 ON c.userIdFrom = u1.userId " +
            "WHERE (userIdFrom = 1 AND (chatVisible = 'B' OR chatVisible = 'F')) " + 
            "OR (userIdTo = 1 AND (chatVisible = 'B' OR chatVisible = 'T')) ORDER BY chatDateTime DESC ; ",
            // Chat visible contains a char = 'B' for both, 'F' from, 'T' to and 'N' none
    chatBunch: "SELECT * FROM chat_message WHERE chatId = ? ORDER BY messageDateTime DESC ; ",
